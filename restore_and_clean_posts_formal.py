...
CLEAN_EXPLANATIONS = {
    '134': {
        ...
        'intro': 'IntelliJ IDEA를 새로 설치하거나 프로젝트를 구성할 때 매번 찾아보는 핵심 필수 설정을 정리했습니다.',
...
| Import 정리 | `Ctrl + Alt + O` | `Cmd + Option + O` |"""
    ...
    '130': {
        ...
        'intro': '로드밸런서 전면이나 Nginx 프록시 환경에서 내부 IP로 직접 요청을 보낼 때, 커스텀 Host 헤더로 가상 호스트 동작을 테스트한 경험을 정리했습니다.',
        'content': """### 사용 명령어 및 옵션
```bash
...
- `-X GET`: GET 메서드로 요청을 전송합니다."""
    ...
    '132': {
        'summary': 'AWS EC2 인스턴스 접속부터 Nginx 설치, SSH 키젠 무암호 접속 및 Java 8, Tomcat 9 WAS 환경 구축 절차입니다.',
        'intro': 'AWS EC2 인스턴스를 최초 생성한 후 개발 및 웹 서비스를 위해 Nginx, Tomcat 9, Java 8 환경을 구축했던 과정을 정리했습니다.',
...
```"""
    ...
    '125': {
        ...
        'intro': 'MS SQL 운영 중 특정 트랜잭션이 테이블 락을 잡고 있어 서비스 응답이 정체되었을 때 빠르게 원인을 찾아 조치한 경험을 정돈했습니다.',
...
def restore_and_build_formal_posts():
    ...
    for item in posts:
        ...
        summary = custom_info.get('summary', f'{title_clean} 작업 시 검증했던 핵심 내용과 설정 가이드를 정리해둔 노트입니다.')
        intro = custom_info.get('intro', f'{title_clean}과 관련해 개발 및 인프라 운용 과정에서 알게 된 점들을 공유합니다.')
        ...
        # Build clean formal markdown structure (~입니다., 공유합니다.)
        body = f"""> **[핵심 요약]**
> {summary}
...
"""
    ...
    print(f"Successfully converted all {restored_count} posts to formal tone (~입니다, 공유합니다)!")

if __name__ == '__main__':
    restore_and_build_formal_posts()
