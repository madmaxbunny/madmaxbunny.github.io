...
> **[핵심 요약]**
> curl 명령어로 커스텀 Host 헤더를 지정하고 HTTP GET 요청으로 헬스체크 및 도메인 라우팅을 테스트하는 방법이에요.

---

## 1. 개요 및 배경

로드밸런서 전면이나 Nginx 프록시 환경에서 내부 IP로 직접 요청을 보낼 때, 커스텀 Host 헤더로 가상 호스트 동작을 테스트한 경험을 정리해봤어요.
